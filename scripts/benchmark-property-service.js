...
async function benchmark() {
  ...
  console.log(`  ✅ Fetched ${properties1?.length || 0} properties in ${t1Time.toFixed(2)}ms`)
  ...
  const { data: properties2 } = await supabase.rpc('get_properties_within_radius', {
    center_lat: 37.7749,
    center_lng: -122.4194,
    radius_km: 5,
    result_limit: 50
  ...
  console.log(`  ✅ Found ${properties2?.length || 0} properties in ${t2Time.toFixed(2)}ms`)
  ...
  console.log(`  ✅ Found ${properties3?.length || 0} properties in ${t3Time.toFixed(2)}ms`)
  ...
  results.operations.forEach(op => {
    ...
    console.log(`  ${op.name.padEnd(20)} ${op.time.toFixed(2).padStart(8)}ms ${bar}`)
  })
