...
// VibesService.generateSourceHash expects non-null bedrooms/bathrooms
// (PropertySource). The generated DB Row type now marks `bathrooms` nullable,
// so coalesce null → 0 here. 0 is the schema's documented unknown/missing
// sentinel for these columns, and real (non-null) rows hash identically.
function toSourceHashInput(
  p: PropertyRow
): Parameters<typeof VibesService.generateSourceHash>[0] {
  return {
    address: p.address,
    city: p.city,
...
async function main() {
  ...
  for (const p of rows) {
    ...
    const currentHash = VibesService.generateSourceHash(toSourceHashInput(p))
  ...
  const staleCount = rows.filter((p) => {
    ...
    return sourceHash !== VibesService.generateSourceHash(toSourceHashInput(p))
  }).length
