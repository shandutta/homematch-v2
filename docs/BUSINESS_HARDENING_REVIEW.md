...
| Auth                 | Improved           | Stay on Supabase Auth; isolate custom policy first. API auth now has a shared `requireUserFromRequest` helper with unit tests for bearer-token fallback and standardized 401 responses. A dead mock `withAuth` middleware that always injected `mock-user-id` was removed after search proved it had no callsites. Middleware still owns page redirects/cookie refresh. | Convert avatar auth and keep third-party auth as a later, planned migration only. |
