...
| API routes           | Improved           | 26 API routes. New `requireUserFromRequest` API boundary standardizes bearer-token fallback and `UNAUTHORIZED` response shape. Activity, user search, property vibes, and neighborhood vibes now use it. Remaining direct `supabase.auth.getUser()` calls are isolated in notify/disputed/avatar/interactions/reset. | Continue converting remaining authenticated routes and add route-level tests for each conversion.  |
