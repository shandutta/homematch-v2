...
export async function GET(request: NextRequest) {
  ...
  if (type === 'summary') {
    ...
    const nextCursor =
      items.length === limit ? data[data.length - 1].created_at : null
...
export function useInfiniteInteractions(type: InteractionType) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      InteractionService.getInteractions(type, { cursor: pageParam }),
...
const StatTile = ({
  ...
  <Link
    href={href}
    className="block rounded-lg bg-white/10 p-6 shadow-lg backdrop-blur-sm transition-colors hover:bg-white/20"
  >
    <div className="text-4xl font-bold text-white">
      {value ?? <Skeleton className="h-10 w-16 bg-white/20" />}
    </div>
...
export function PropertyCard({ property, onDecision }: PropertyCardProps) {
  ...
  return (
    <div className="card-glassmorphism-style relative">
      ...
      <div className="absolute bottom-4 flex w-full justify-center space-x-4">
        <button
          onClick={() => onDecision(property.id, 'skip')}
          data-testid="pass-button"
        >
          Pass
        </button>
        <button
          onClick={() => onDecision(property.id, 'liked')}
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {properties.map((property) => (
          ...
          <div key={property.id} className="rounded-lg bg-white/10 p-4">
  ...
  .gradient-grid-bg {
    ...
    background-image:
      ...
      radial-gradient(
        ...
        rgba(2, 26, 68, 0.06) 0%,
        rgba(2, 26, 68, 0.03) 35%,
        rgba(255, 255, 255, 0) 65%
      ),
      /* Layer 2: Repeating grid pattern */
        repeating-linear-gradient(
          0deg,
          rgba(2, 26, 68, 0.08) 0px,
          rgba(2, 26, 68, 0.08) 1px,
          transparent 1px,
          transparent 28px
        ),
      repeating-linear-gradient(
      ...
      /* Layer 3: Additional colored radial glows */
        radial-gradient(
          600px 300px at 80% 0%,
          rgba(41, 227, 255, 0.12) 0%,
          rgba(41, 227, 255, 0) 60%
        ),
      radial-gradient(
        700px 320px at 15% 0%,
        rgba(6, 58, 158, 0.1) 0%,
...
export default function DashboardLayout({
  ...
  return (
    <div className="gradient-grid-bg flex min-h-screen flex-col">
      <Header />
...
2.  **Database Functions & Policies (`__tests__/integration/database/interactions.test.ts`)**
    ...
    - **RLS Policy for `user_property_interactions`**:
      ...
      - Verify that a user _cannot_ insert an interaction with a `user_id` belonging to another user (this should be blocked by the RLS policy).
