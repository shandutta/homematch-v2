...
- **Scheduled trigger (template)**: `vercel.cron.template.json` shows how to schedule `/api/admin/ingest/zillow` daily at 09:00 UTC. If Vercel Cron cannot set headers, append `?cron_secret=${ZILLOW_CRON_SECRET}` to the path; otherwise send header `x-cron-secret`.
- **Secure endpoint**: `POST /api/admin/ingest/zillow` (requires `x-cron-secret` header or `cron_secret` query matching `ZILLOW_CRON_SECRET`; uses `RAPIDAPI_KEY` and Supabase service role). Fails with 401 if the secret is missing.
