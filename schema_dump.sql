...
CREATE OR REPLACE FUNCTION "public"."calculate_distance"("lat1" double precision, "lng1" double precision, "lat2" double precision, "lng2" double precision) RETURNS double precision
...
ALTER FUNCTION "public"."calculate_distance"("lat1" double precision, "lng1" double precision, "lat2" double precision, "lng2" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."calculate_distance"("lat1" double precision, "lng1" double precision, "lat2" double precision, "lng2" double precision) IS 'Calculates distance in kilometers between two points using PostGIS';



CREATE OR REPLACE FUNCTION "public"."check_potential_mutual_like"("p_user_id" "uuid", "p_property_id" "uuid", "p_household_id" "uuid") RETURNS TABLE("would_be_mutual" boolean, "partner_user_id" "uuid")
...
ALTER FUNCTION "public"."check_potential_mutual_like"("p_user_id" "uuid", "p_property_id" "uuid", "p_household_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."check_table_exists"("table_names" "text"[]) RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  result json;
...
ALTER FUNCTION "public"."check_table_exists"("table_names" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."geocode_address"("address_text" "text") RETURNS TABLE("latitude" double precision, "longitude" double precision, "formatted_address" "text", "confidence" numeric)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."geocode_address"("address_text" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."geocode_address"("address_text" "text") IS 'Stub function for address geocoding - to be implemented with geocoding service';



CREATE OR REPLACE FUNCTION "public"."get_geographic_density"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "grid_size_deg" double precision DEFAULT 0.01) RETURNS TABLE("total_properties" bigint, "avg_price" numeric, "price_density" "jsonb")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_geographic_density"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "grid_size_deg" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_geographic_density"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "grid_size_deg" double precision) IS 'Returns property density analysis within a bounding box using a grid system';



CREATE OR REPLACE FUNCTION "public"."get_household_activity_enhanced"("p_household_id" "uuid", "p_limit" integer DEFAULT 20, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "user_id" "uuid", "property_id" "uuid", "interaction_type" "text", "created_at" timestamp with time zone, "user_display_name" "text", "user_email" "text", "property_address" "text", "property_price" numeric, "property_bedrooms" integer, "property_bathrooms" numeric, "property_images" "text"[])
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  SELECT 
...
ALTER FUNCTION "public"."get_household_activity_enhanced"("p_household_id" "uuid", "p_limit" integer, "p_offset" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_household_mutual_likes"("p_household_id" "uuid") RETURNS TABLE("property_id" "uuid", "liked_by_count" bigint, "first_liked_at" timestamp with time zone, "last_liked_at" timestamp with time zone, "user_ids" "uuid"[])
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  SELECT 
...
ALTER FUNCTION "public"."get_household_mutual_likes"("p_household_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_market_trends"("timeframe" "text" DEFAULT 'monthly'::"text", "months_back" integer DEFAULT 12) RETURNS TABLE("period" "text", "avg_price" numeric, "total_listings" bigint, "price_change_percent" numeric)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_market_trends"("timeframe" "text", "months_back" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_market_trends"("timeframe" "text", "months_back" integer) IS 'Returns market trends over specified timeframe with price change percentages';



CREATE OR REPLACE FUNCTION "public"."get_market_velocity"("target_neighborhood_id" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("avg_days_on_market" numeric, "total_sold" bigint, "velocity_score" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_market_velocity"("target_neighborhood_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_market_velocity"("target_neighborhood_id" "uuid") IS 'Calculates market velocity (how quickly properties sell) for a neighborhood or overall market';



CREATE OR REPLACE FUNCTION "public"."get_nearest_amenities"("center_lat" double precision, "center_lng" double precision, "amenity_types" "text"[], "search_radius_km" double precision DEFAULT 2) RETURNS TABLE("amenity_id" "text", "amenity_name" "text", "amenity_type" "text", "distance_km" double precision, "latitude" double precision, "longitude" double precision)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."get_nearest_amenities"("center_lat" double precision, "center_lng" double precision, "amenity_types" "text"[], "search_radius_km" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_nearest_amenities"("center_lat" double precision, "center_lng" double precision, "amenity_types" "text"[], "search_radius_km" double precision) IS 'Stub function for finding nearby amenities - to be implemented with POI database';



CREATE OR REPLACE FUNCTION "public"."get_neighborhood_stats"("neighborhood_uuid" "uuid") RETURNS TABLE("neighborhood_id" "uuid", "neighborhood_name" "text", "neighborhood_city" "text", "neighborhood_state" "text", "total_properties" bigint, "avg_price" numeric, "median_price" numeric, "min_price" integer, "max_price" integer, "avg_bedrooms" numeric, "avg_bathrooms" numeric, "avg_square_feet" numeric, "property_type_distribution" "jsonb")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_neighborhood_stats"("neighborhood_uuid" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_neighborhood_stats"("neighborhood_uuid" "uuid") IS 'Returns comprehensive statistics for a specific neighborhood';



CREATE OR REPLACE FUNCTION "public"."get_neighborhoods_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision) RETURNS TABLE("id" "uuid", "name" "text", "city" "text", "state" "text", "metro_area" "text", "bounds" "public"."geometry", "median_price" integer, "walk_score" integer, "transit_score" integer, "created_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_neighborhoods_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_neighborhoods_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision) IS 'Returns neighborhoods that intersect with a bounding box';



CREATE OR REPLACE FUNCTION "public"."get_properties_along_route"("waypoints" "jsonb", "corridor_width_km" double precision DEFAULT 1) RETURNS TABLE("id" "uuid", "address" "text", "city" "text", "state" "text", "price" integer, "bedrooms" integer, "bathrooms" numeric, "property_type" "text", "neighborhood_id" "uuid", "distance_from_route" double precision)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_properties_along_route"("waypoints" "jsonb", "corridor_width_km" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_properties_along_route"("waypoints" "jsonb", "corridor_width_km" double precision) IS 'Returns properties within a corridor along a route defined by waypoints';



CREATE OR REPLACE FUNCTION "public"."get_properties_by_distance"("center_lat" double precision, "center_lng" double precision, "max_distance_km" double precision DEFAULT 10, "result_limit" integer DEFAULT 20) RETURNS TABLE("id" "uuid", "address" "text", "city" "text", "state" "text", "price" integer, "bedrooms" integer, "bathrooms" numeric, "square_feet" integer, "property_type" "text", "images" "text"[], "neighborhood_id" "uuid", "distance_km" double precision)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_properties_by_distance"("center_lat" double precision, "center_lng" double precision, "max_distance_km" double precision, "result_limit" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_properties_by_distance"("center_lat" double precision, "center_lng" double precision, "max_distance_km" double precision, "result_limit" integer) IS 'Returns properties sorted by distance from a center point';



CREATE OR REPLACE FUNCTION "public"."get_properties_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "result_limit" integer DEFAULT 100) RETURNS TABLE("id" "uuid", "address" "text", "city" "text", "state" "text", "zip_code" "text", "price" integer, "bedrooms" integer, "bathrooms" numeric, "square_feet" integer, "property_type" "text", "images" "text"[], "description" "text", "coordinates" "public"."geometry", "neighborhood_id" "uuid", "amenities" "text"[], "year_built" integer, "lot_size_sqft" integer, "parking_spots" integer, "listing_status" "text", "is_active" boolean, "created_at" timestamp with time zone, "updated_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_properties_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "result_limit" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_properties_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "result_limit" integer) IS 'Returns properties within a bounding box';



CREATE OR REPLACE FUNCTION "public"."get_properties_in_polygon"("polygon_points" "jsonb", "result_limit" integer DEFAULT 100) RETURNS TABLE("id" "uuid", "address" "text", "city" "text", "state" "text", "price" integer, "bedrooms" integer, "bathrooms" numeric, "square_feet" integer, "property_type" "text", "images" "text"[], "coordinates" "public"."geometry", "neighborhood_id" "uuid")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_properties_in_polygon"("polygon_points" "jsonb", "result_limit" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_properties_in_polygon"("polygon_points" "jsonb", "result_limit" integer) IS 'Returns properties within a custom polygon defined by coordinate points';
...
CREATE TABLE IF NOT EXISTS "public"."properties" (
    ...
    CONSTRAINT "properties_property_type_check" CHECK (("property_type" = ANY (ARRAY['single_family'::"text", 'condo'::"text", 'townhome'::"text", 'multi_family'::"text", 'manufactured'::"text", 'land'::"text", 'other'::"text"])))
);
...
COMMENT ON COLUMN "public"."properties"."coordinates" IS 'PostGIS point geometry representing property location in WGS84 (SRID 4326)';



CREATE OR REPLACE FUNCTION "public"."get_properties_within_radius"("center_lat" double precision, "center_lng" double precision, "radius_km" double precision, "result_limit" integer DEFAULT 50) RETURNS SETOF "public"."properties"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."get_properties_within_radius"("center_lat" double precision, "center_lng" double precision, "radius_km" double precision, "result_limit" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_properties_within_radius"("center_lat" double precision, "center_lng" double precision, "radius_km" double precision, "result_limit" integer) IS 'Returns properties within a specified radius (in kilometers) from a center point, ordered by distance';



CREATE OR REPLACE FUNCTION "public"."get_property_clusters"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "zoom_level" integer DEFAULT 10) RETURNS TABLE("lat" double precision, "lng" double precision, "count" bigint, "avg_price" numeric, "min_price" integer, "max_price" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_property_clusters"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "zoom_level" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_property_clusters"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "zoom_level" integer) IS 'Returns property clusters for map visualization based on zoom level';



CREATE OR REPLACE FUNCTION "public"."get_property_market_comparisons"("target_property_id" "uuid", "radius_km" double precision DEFAULT 5) RETURNS TABLE("id" "uuid", "address" "text", "city" "text", "state" "text", "price" integer, "bedrooms" integer, "bathrooms" numeric, "square_feet" integer, "property_type" "text", "neighborhood_id" "uuid", "price_per_sqft" numeric, "distance_km" double precision)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_property_market_comparisons"("target_property_id" "uuid", "radius_km" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_property_market_comparisons"("target_property_id" "uuid", "radius_km" double precision) IS 'Returns market comparisons for a property within specified radius';



CREATE OR REPLACE FUNCTION "public"."get_similar_properties"("target_property_id" "uuid", "radius_km" double precision DEFAULT 5, "result_limit" integer DEFAULT 10) RETURNS TABLE("id" "uuid", "address" "text", "city" "text", "state" "text", "price" integer, "bedrooms" integer, "bathrooms" numeric, "square_feet" integer, "property_type" "text", "images" "text"[], "neighborhood_id" "uuid", "similarity_score" numeric, "distance_km" double precision)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_similar_properties"("target_property_id" "uuid", "radius_km" double precision, "result_limit" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_similar_properties"("target_property_id" "uuid", "radius_km" double precision, "result_limit" integer) IS 'Finds similar properties based on characteristics and location';



CREATE OR REPLACE FUNCTION "public"."get_transit_score"("center_lat" double precision, "center_lng" double precision) RETURNS integer
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_transit_score"("center_lat" double precision, "center_lng" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_transit_score"("center_lat" double precision, "center_lng" double precision) IS 'Calculates transit score based on neighborhood data and property density';



CREATE OR REPLACE FUNCTION "public"."get_user_interaction_summary"("p_user_id" "uuid") RETURNS TABLE("interaction_type" "text", "count" bigint)
    LANGUAGE "sql" STABLE
...
COMMENT ON FUNCTION "public"."get_user_interaction_summary"("p_user_id" "uuid") IS 'Aggregates interaction counts by type for the specified user_id. search_path hardened to pg_catalog, public.';



CREATE OR REPLACE FUNCTION "public"."get_walkability_score"("center_lat" double precision, "center_lng" double precision) RETURNS integer
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_walkability_score"("center_lat" double precision, "center_lng" double precision) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_walkability_score"("center_lat" double precision, "center_lng" double precision) IS 'Calculates walkability score based on property density and neighborhood data';
...
CREATE OR REPLACE FUNCTION "public"."is_test_environment"() RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
...
ALTER FUNCTION "public"."is_test_environment"() OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."households" (
    "id" "uuid" DEFAULT "extensions"."uuid_generate_v4"() NOT NULL,
    "name" "text" NOT NULL,
    "collaboration_mode" "text" DEFAULT 'independent'::"text",
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "households_collaboration_mode_check" CHECK (("collaboration_mode" = ANY (ARRAY['independent'::"text", 'shared'::"text", 'weighted'::"text"])))
);
...
COMMENT ON COLUMN "public"."neighborhoods"."bounds" IS 'PostGIS polygon geometry representing neighborhood boundaries in WGS84 (SRID 4326)';
...
CREATE TABLE IF NOT EXISTS "public"."user_property_interactions" (
    "id" "uuid" DEFAULT "extensions"."uuid_generate_v4"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    ...
    "score_data" "jsonb",
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "user_property_interactions_interaction_type_check" CHECK (("interaction_type" = ANY (ARRAY['like'::"text", 'dislike'::"text", 'skip'::"text", 'view'::"text"])))
);


ALTER TABLE "public"."user_property_interactions" OWNER TO "postgres";
...
COMMENT ON TABLE "public"."users" IS 'Mirror of auth.users for test compatibility and foreign key references';
...
ALTER TABLE ONLY "public"."user_property_interactions"
    ADD CONSTRAINT "user_property_interactions_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."user_property_interactions"
    ADD CONSTRAINT "user_property_interactions_user_id_property_id_interaction__key" UNIQUE ("user_id", "property_id", "interaction_type");
...
CREATE INDEX "idx_neighborhoods_bounds" ON "public"."neighborhoods" USING "gist" ("bounds");



CREATE INDEX "idx_neighborhoods_city_state" ON "public"."neighborhoods" USING "btree" ("city", "state");
...
CREATE INDEX "idx_properties_coordinates_geography" ON "public"."properties" USING "gist" ((("coordinates")::"public"."geography"));
...
CREATE INDEX "idx_properties_square_feet" ON "public"."properties" USING "btree" ("square_feet");



CREATE INDEX "idx_saved_searches_household_id" ON "public"."saved_searches" USING "btree" ("household_id");
...
CREATE INDEX "idx_user_property_interactions_household_id" ON "public"."user_property_interactions" USING "btree" ("household_id");



CREATE INDEX "idx_user_property_interactions_household_property_type" ON "public"."user_property_interactions" USING "btree" ("household_id", "property_id", "interaction_type");



CREATE INDEX "idx_user_property_interactions_household_type_created" ON "public"."user_property_interactions" USING "btree" ("household_id", "interaction_type", "created_at" DESC);



CREATE INDEX "idx_user_property_interactions_household_user_type" ON "public"."user_property_interactions" USING "btree" ("household_id", "user_id", "interaction_type");



CREATE INDEX "idx_user_property_interactions_property_id" ON "public"."user_property_interactions" USING "btree" ("property_id");



CREATE INDEX "idx_user_property_interactions_type" ON "public"."user_property_interactions" USING "btree" ("interaction_type");



CREATE INDEX "idx_user_property_interactions_user_id" ON "public"."user_property_interactions" USING "btree" ("user_id");
...
ALTER TABLE ONLY "public"."user_profiles"
    ADD CONSTRAINT "fk_user_profiles_household" FOREIGN KEY ("household_id") REFERENCES "public"."households"("id");



ALTER TABLE ONLY "public"."properties"
    ADD CONSTRAINT "properties_neighborhood_id_fkey" FOREIGN KEY ("neighborhood_id") REFERENCES "public"."neighborhoods"("id");



ALTER TABLE ONLY "public"."saved_searches"
    ADD CONSTRAINT "saved_searches_household_id_fkey" FOREIGN KEY ("household_id") REFERENCES "public"."households"("id");
...
COMMENT ON CONSTRAINT "saved_searches_user_id_fkey" ON "public"."saved_searches" IS 'Foreign key to users table (test-compatible) with CASCADE delete for proper test cleanup';
...
COMMENT ON CONSTRAINT "user_profiles_id_fkey" ON "public"."user_profiles" IS 'Foreign key to users table (test-compatible) with CASCADE delete for proper test cleanup';



ALTER TABLE ONLY "public"."user_property_interactions"
    ADD CONSTRAINT "user_property_interactions_household_id_fkey" FOREIGN KEY ("household_id") REFERENCES "public"."households"("id");



ALTER TABLE ONLY "public"."user_property_interactions"
    ADD CONSTRAINT "user_property_interactions_property_id_fkey" FOREIGN KEY ("property_id") REFERENCES "public"."properties"("id");



ALTER TABLE ONLY "public"."user_property_interactions"
    ADD CONSTRAINT "user_property_interactions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



COMMENT ON CONSTRAINT "user_property_interactions_user_id_fkey" ON "public"."user_property_interactions" IS 'Foreign key to users table (test-compatible) with CASCADE delete for proper test cleanup';
...
CREATE POLICY "Authenticated users can create households for testing" ON "public"."households" FOR INSERT WITH CHECK (("auth"."uid"() IS NOT NULL));



COMMENT ON POLICY "Authenticated users can create households for testing" ON "public"."households" IS 'TEST ONLY: Remove for production';



CREATE POLICY "Authenticated users can delete properties for testing" ON "public"."properties" FOR DELETE USING (("auth"."uid"() IS NOT NULL));



COMMENT ON POLICY "Authenticated users can delete properties for testing" ON "public"."properties" IS 'TEST ONLY: Remove for production';



CREATE POLICY "Authenticated users can insert properties for testing" ON "public"."properties" FOR INSERT WITH CHECK (("auth"."uid"() IS NOT NULL));



COMMENT ON POLICY "Authenticated users can insert properties for testing" ON "public"."properties" IS 'TEST ONLY: Remove for production';



CREATE POLICY "Authenticated users can update properties for testing" ON "public"."properties" FOR UPDATE USING (("auth"."uid"() IS NOT NULL));



COMMENT ON POLICY "Authenticated users can update properties for testing" ON "public"."properties" IS 'TEST ONLY: Remove for production';



CREATE POLICY "Household members can access mutual likes function" ON "public"."user_property_interactions" FOR SELECT USING (("household_id" IN ( SELECT "user_profiles"."household_id"
   FROM "public"."user_profiles"
  WHERE ("user_profiles"."id" = "auth"."uid"()))));



CREATE POLICY "Household members can view all household interactions" ON "public"."user_property_interactions" FOR SELECT USING (("household_id" IN ( SELECT "user_profiles"."household_id"
   FROM "public"."user_profiles"
  WHERE (("user_profiles"."id" = "auth"."uid"()) AND ("user_profiles"."household_id" IS NOT NULL)))));



CREATE POLICY "Service role can manage all user profiles" ON "public"."user_profiles" USING ((("auth"."jwt"() ->> 'role'::"text") = 'service_role'::"text")) WITH CHECK ((("auth"."jwt"() ->> 'role'::"text") = 'service_role'::"text"));
...
CREATE POLICY "Users can insert interactions with household_id" ON "public"."user_property_interactions" FOR INSERT WITH CHECK ((("auth"."uid"() = "user_id") AND (("household_id" IS NULL) OR ("household_id" IN ( SELECT "user_profiles"."household_id"
   FROM "public"."user_profiles"
  WHERE ("user_profiles"."id" = "auth"."uid"()))))));
...
CREATE POLICY "Users can insert their own interactions" ON "public"."user_property_interactions" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can update their household" ON "public"."households" FOR UPDATE USING (("id" IN ( SELECT "user_profiles"."household_id"
   FROM "public"."user_profiles"
  WHERE ("user_profiles"."id" = "auth"."uid"()))));



CREATE POLICY "Users can update their own interactions" ON "public"."user_property_interactions" FOR UPDATE USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can update their own searches" ON "public"."saved_searches" FOR UPDATE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can view own profile" ON "public"."user_profiles" FOR SELECT USING ((("auth"."uid"() = "id") OR (("auth"."jwt"() ->> 'role'::"text") = 'service_role'::"text")));



CREATE POLICY "Users can view their household" ON "public"."households" FOR SELECT USING (("id" IN ( SELECT "user_profiles"."household_id"
   FROM "public"."user_profiles"
  WHERE ("user_profiles"."id" = "auth"."uid"()))));



CREATE POLICY "Users can view their own interactions" ON "public"."user_property_interactions" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "properties_anon_marketing_read" ON "public"."properties" FOR SELECT TO "anon" USING ((("listing_status" = 'active'::"text") AND (COALESCE("is_active", true) = true)));
...
ALTER TABLE "public"."user_property_interactions" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."calculate_distance"("lat1" double precision, "lng1" double precision, "lat2" double precision, "lng2" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_distance"("lat1" double precision, "lng1" double precision, "lat2" double precision, "lng2" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_distance"("lat1" double precision, "lng1" double precision, "lat2" double precision, "lng2" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."check_potential_mutual_like"("p_user_id" "uuid", "p_property_id" "uuid", "p_household_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."check_potential_mutual_like"("p_user_id" "uuid", "p_property_id" "uuid", "p_household_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_potential_mutual_like"("p_user_id" "uuid", "p_property_id" "uuid", "p_household_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."check_table_exists"("table_names" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."check_table_exists"("table_names" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_table_exists"("table_names" "text"[]) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."geocode_address"("address_text" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."geocode_address"("address_text" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."geocode_address"("address_text" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_geographic_density"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "grid_size_deg" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."get_geographic_density"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "grid_size_deg" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_geographic_density"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "grid_size_deg" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_household_activity_enhanced"("p_household_id" "uuid", "p_limit" integer, "p_offset" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_household_activity_enhanced"("p_household_id" "uuid", "p_limit" integer, "p_offset" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_household_activity_enhanced"("p_household_id" "uuid", "p_limit" integer, "p_offset" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_household_mutual_likes"("p_household_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_household_mutual_likes"("p_household_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_household_mutual_likes"("p_household_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_market_trends"("timeframe" "text", "months_back" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_market_trends"("timeframe" "text", "months_back" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_market_trends"("timeframe" "text", "months_back" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_market_velocity"("target_neighborhood_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_market_velocity"("target_neighborhood_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_market_velocity"("target_neighborhood_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_nearest_amenities"("center_lat" double precision, "center_lng" double precision, "amenity_types" "text"[], "search_radius_km" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."get_nearest_amenities"("center_lat" double precision, "center_lng" double precision, "amenity_types" "text"[], "search_radius_km" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_nearest_amenities"("center_lat" double precision, "center_lng" double precision, "amenity_types" "text"[], "search_radius_km" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_neighborhood_stats"("neighborhood_uuid" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_neighborhood_stats"("neighborhood_uuid" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_neighborhood_stats"("neighborhood_uuid" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_neighborhoods_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."get_neighborhoods_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_neighborhoods_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_properties_along_route"("waypoints" "jsonb", "corridor_width_km" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."get_properties_along_route"("waypoints" "jsonb", "corridor_width_km" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_properties_along_route"("waypoints" "jsonb", "corridor_width_km" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_properties_by_distance"("center_lat" double precision, "center_lng" double precision, "max_distance_km" double precision, "result_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_properties_by_distance"("center_lat" double precision, "center_lng" double precision, "max_distance_km" double precision, "result_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_properties_by_distance"("center_lat" double precision, "center_lng" double precision, "max_distance_km" double precision, "result_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_properties_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "result_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_properties_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "result_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_properties_in_bounds"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "result_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_properties_in_polygon"("polygon_points" "jsonb", "result_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_properties_in_polygon"("polygon_points" "jsonb", "result_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_properties_in_polygon"("polygon_points" "jsonb", "result_limit" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_properties_within_radius"("center_lat" double precision, "center_lng" double precision, "radius_km" double precision, "result_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_properties_within_radius"("center_lat" double precision, "center_lng" double precision, "radius_km" double precision, "result_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_properties_within_radius"("center_lat" double precision, "center_lng" double precision, "radius_km" double precision, "result_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_property_clusters"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "zoom_level" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_property_clusters"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "zoom_level" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_property_clusters"("north_lat" double precision, "south_lat" double precision, "east_lng" double precision, "west_lng" double precision, "zoom_level" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_property_market_comparisons"("target_property_id" "uuid", "radius_km" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."get_property_market_comparisons"("target_property_id" "uuid", "radius_km" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_property_market_comparisons"("target_property_id" "uuid", "radius_km" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_similar_properties"("target_property_id" "uuid", "radius_km" double precision, "result_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_similar_properties"("target_property_id" "uuid", "radius_km" double precision, "result_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_similar_properties"("target_property_id" "uuid", "radius_km" double precision, "result_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_transit_score"("center_lat" double precision, "center_lng" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."get_transit_score"("center_lat" double precision, "center_lng" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_transit_score"("center_lat" double precision, "center_lng" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_user_interaction_summary"("p_user_id" "uuid") TO "anon";
...
GRANT ALL ON FUNCTION "public"."get_walkability_score"("center_lat" double precision, "center_lng" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."get_walkability_score"("center_lat" double precision, "center_lng" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_walkability_score"("center_lat" double precision, "center_lng" double precision) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."is_test_environment"() TO "anon";
GRANT ALL ON FUNCTION "public"."is_test_environment"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."is_test_environment"() TO "service_role";
...
GRANT ALL ON TABLE "public"."user_property_interactions" TO "anon";
GRANT ALL ON TABLE "public"."user_property_interactions" TO "authenticated";
GRANT ALL ON TABLE "public"."user_property_interactions" TO "service_role";
