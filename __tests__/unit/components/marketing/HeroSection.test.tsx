...
jest.mock('@/components/marketing/ParallaxStarsCanvas', () => ({
  ParallaxStarsCanvas: ({ className }: any) => (
    <div data-testid="parallax-stars" className={className}>Stars</div>
  ),
}))
...
describe('HeroSection', () => {
  test('renders hero section with all content', () => {
    ...
    // Check description
    expect(screen.getByText(/House hunting just became your favorite couples activity/)).toBeInTheDocument()
  ...
  test('renders gradient background', () => {
    ...
    const gradientDiv = screen.getByTestId('hero').querySelector('div[style*="linear-gradient"]')
  ...
  test('renders vignette overlay', () => {
    ...
    const vignetteDiv = screen.getByTestId('hero').querySelector('div[style*="radial-gradient"]')
  ...
  test('CTA container has responsive flex layout', () => {
    ...
    const ctaContainer = screen.getByText('Get started').closest('a')?.parentElement
