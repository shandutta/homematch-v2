...
describe('InteractionsListPage', () => {
  ...
  it('renders empty state when no properties after load', () => {
    ...
    expect(screen.getByText(/Start swiping to see properties here!/i)).toBeTruthy();
  });
  ...
  it('renders PropertyCard for each item when data present', () => {
    mockedUseInfinite.mockReturnValue({
      ...
      data: {
        pages: [
          { items: [{ id: 'p1', address: '123 A', city: 'X', state: 'CA', zip_code: '00000', price: 1, bedrooms: 1, bathrooms: 1, square_feet: 1, property_type: 'house' }] },
          { items: [{ id: 'p2', address: '456 B', city: 'Y', state: 'CA', zip_code: '00000', price: 2, bedrooms: 2, bathrooms: 2, square_feet: 2, property_type: 'condo' }] },
        ],
      },
    });
  ...
  it('shows Load More when hasNextPage, calls fetchNextPage on click', () => {
    ...
    mockedUseInfinite.mockReturnValue({
      ...
      fetchNextPage,
      data: {
        pages: [{ items: [{ id: 'p1', address: '123 A', city: 'X', state: 'CA', zip_code: '00000', price: 1, bedrooms: 1, bathrooms: 1, square_feet: 1, property_type: 'house' }] }],
      },
    });

    render(<InteractionsListPage type="viewed" title="Viewed" />);

    const btn = screen.getByRole('button', { name: /Load More/i }) as HTMLButtonElement;
  ...
  it('disables Load More when isFetchingNextPage', () => {
    ...
    const btn = screen.getByRole('button', { name: /Loading more.../i }) as HTMLButtonElement;
