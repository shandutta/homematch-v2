...
describe('PropertyAnalyticsService', () => {
    ...
    test('should filter by neighborhood when provided', async () => {
      ...
      mockSupabaseClient.eq
        ...
        .mockResolvedValueOnce({
          // Resolve for the second .eq('neighborhood_id', 'neigh-123')
  ...
  describe('getTopPerformingProperties', () => {
    test('should return top performing properties by views', async () => {
      ...
      const result = await analyticsService.getTopPerformingProperties(
        'views',
        5
      )

      expect(result).toEqual(mockProperties)
      expect(mockSupabaseClient.order).toHaveBeenCalledWith('view_count', {
        ascending: false,
      })
    ...
    test('should handle different metrics', async () => {
      ...
      expect(mockSupabaseClient.order).toHaveBeenCalledWith('like_count', {
        ascending: false,
      })

      await analyticsService.getTopPerformingProperties('price')
      expect(mockSupabaseClient.order).toHaveBeenCalledWith('price', {
        ascending: false,
      })
    })
  })
  ...
  describe('getSimilarProperties', () => {
    test('should return similar properties', async () => {
      ...
      const result = await analyticsService.getSimilarProperties(
        'prop-123',
        5,
        10
      )
  ...
  describe('getNeighborhoodPriceTrends', () => {
    test('should return neighborhood price trends using market trends', async () => {
      ...
      const result = await analyticsService.getNeighborhoodPriceTrends(
        'neigh-123',
        6
      )
  ...
  describe('getInvestmentAnalysis', () => {
    test('should provide comprehensive investment analysis', async () => {
      ...
      jest
        .spyOn(analyticsService, 'analyzePropertyPricing')
        .mockResolvedValue(priceAnalysis)
