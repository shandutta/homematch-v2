...
test('Auth Fixtures - should show login page elements', async ({ page, utils, config, logger }) => {
  ...
  await expect(page.locator('p').first()).toContainText('Sign in to your account')
...
test('Auth Fixtures - should enable button when form is filled', async ({ page, utils, config, logger }) => {
  ...
  await expect(submitButton).toBeEnabled({ timeout: config.timeouts.BUTTON_ENABLED })
