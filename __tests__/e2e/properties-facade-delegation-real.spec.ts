...
test.describe('Property Service Facade UI Tests - Real Browser', () => {
    ...
    test('should handle service failures gracefully', async ({
      ...
      try {
        ...
        const matchScore = firstProperty
          .locator(
            '[data-testid="match-score"], .match-score, [class*="score"]'
          )
          .first()
        try {
          ...
          logger.info(
            'Match score element not accessible during service failure'
          )
        }

        logger.info('Facade handled service failure gracefully')
      } catch (error) {
        // If dashboard content isn't loading, check if we're on login page
  ...
  test.describe('Error Handling in Facade Pattern', () => {
    test('should provide fallback data when services are unavailable', async ({
      ...
      // Properties should still load with fallback data - wait for dashboard to be fully loaded
      try {
        await page.waitForSelector('[data-testid="dashboard-content"]', {
          timeout: 30000,
        })
