...
test.describe('Household Clipboard Functionality', () => {
  ...
  test('copies household code to clipboard when copy button is clicked', async ({ page }) => {
  ...
  test('clipboard functionality works across different browsers', async ({ page, browserName }) => {
  ...
  test('handles clipboard permissions gracefully', async ({ page, context }) => {
  ...
  test('copy functionality works in complete user workflow', async ({ page }) => {
    await page.goto('/profile')
    await page.waitForLoadState('networkidle')
    ...
    await expect(page.locator('text=Share this code with family members')).toBeVisible()
    ...
    const clipboardText = await page.evaluate(() => navigator.clipboard.readText())
    expect(clipboardText).toBe(householdId)
    await expect(page.locator('[data-testid="toast-success"]')).toContainText('copied to clipboard', { timeout: 5000 })
    
    // Verify other household functionality is still accessible
    await expect(page.locator('button:has-text("Leave Household")')).toBeVisible()
  })
