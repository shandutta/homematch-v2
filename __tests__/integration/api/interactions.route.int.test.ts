 ...
 *
 * If you run a dev server in CI for integration tests, set BASE_URL (e.g., http://localhost:3000)
...
describe('Integration: /api/interactions (authenticated paths)', () => {
  ...
  it('GET ?type=viewed (cursor/limit) returns 200 with items and nextCursor', async () => {
    ...
    const { status, body } = await fetchJson('/api/interactions?type=viewed&cursor=&limit=10');
