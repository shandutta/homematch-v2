...
describe('Household Functionality Tests', () => {
  ...
  describe('Household Creation and Management', () => {
    ...
    test('should create household and associate users', async () => {
      ...
      if (profile1Error || profile2Error) {
        throw profile1Error || profile2Error
      }
    ...
    test('should find household members correctly', async () => {
      ...
      if (householdError) {
        throw new Error(
          `Could not create household for members test: ${householdError.message}`
        )
      }
  ...
  describe('Household Property Interactions', () => {
    ...
    test('should track household interactions correctly', async () => {
      ...
      if (householdError) {
        throw new Error(
          `Could not create household for interactions test: ${householdError.message}`
        )
      }
      ...
      const { data: householdInteractions, error: queryError } = await supabase
        .from('user_property_interactions')
        .select('*')
        .eq('household_id', testHouseholdId)
    ...
    test('should handle different interaction types', async () => {
      ...
      if (householdError) {
        throw new Error(
          `Could not create household for types test: ${householdError.message}`
        )
      }
