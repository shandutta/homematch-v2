    ...
    const auth: AuthFixture = {
        ...
        async login(user: TestUser = config.users.user1) {
          ...
          if (!pText?.includes('Sign in to your account')) {
            throw new Error(`Expected p to contain 'Sign in to your account', got: ${pText}`)
          }
          ...
          try {
            ...
            const alertSelector = '.alert-destructive, [role="alert"], .error-message'
            const errorAlert = page.locator(alertSelector).first()
            const isErrorVisible = await errorAlert.isVisible({ timeout: 1000 }).catch(() => false)
            ...
            if (currentUrl.includes('/login')) {
              throw new Error(`Authentication failed: Still on login page. Check test user credentials.`)
            }
        ...
        async logout() {
          const signOutButton = page.locator('button[type="submit"]:has-text("Sign out")')
          ...
          try {
            ...
            await page.waitForURL('/login', { timeout: config.timeouts.AUTH_LOGOUT })
          }
        ...
        async verifyAuthenticated(user: TestUser = config.users.user1) {
          ...
          await page.waitForURL(/.*\/validation/, { timeout: config.timeouts.NAVIGATION })
          ...
          while (attempts < maxAttempts) {
            const h1Text = await page.locator('h1').textContent()
            if (h1Text?.includes('HomeMatch V2 - Database Migration Validation')) break
            await page.waitForTimeout(100)
            attempts++
          }
          const h1Text = await page.locator('h1').textContent()
          if (!h1Text?.includes('HomeMatch V2 - Database Migration Validation')) {
            throw new Error(`Expected h1 to contain validation text, got: ${h1Text}`)
          }
          
          // Verify user email is displayed
          attempts = 0
          while (attempts < maxAttempts) {
            const emailVisible = await page.locator(`text=${user.email}`).first().isVisible()
          ...
          const emailVisible = await page.locator(`text=${user.email}`).first().isVisible()
          if (!emailVisible) {
            throw new Error(`User email ${user.email} is not visible on validation dashboard`)
          }
        },
        ...
        async verifyNotAuthenticated() {
          ...
          if (finalUrl.includes('/login')) {
            ...
            const noAuthMessage = await page.locator('text=No Authenticated User').isVisible()
            
            if (!loginLinkVisible && !noAuthMessage) {
              throw new Error('Expected either login link or no auth message to be visible')
            }
          }
        },
