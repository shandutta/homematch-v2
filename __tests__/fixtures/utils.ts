...
export const utilsFixtures = {
  utils: async ({ page }, use) => {
    const utils: UtilsFixture = {
      ...
      async waitForAuthRedirect(expectedUrl: string | RegExp, options?: { timeout?: number, errorMessage?: string }) {
        const { timeout = TEST_TIMEOUTS.AUTH_REDIRECT, errorMessage = 'Failed to redirect after authentication' } = options || {}
