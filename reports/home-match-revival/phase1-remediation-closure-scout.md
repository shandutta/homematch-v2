...
| M6: Standardize API error handling | `ApiErrorHandler` now covers shared helpers plus interactions, Zillow random-image, couples/vibes, paid Maps proxy, remaining couples activity/notify/disputed, and admin cron/ingest/vibes route families. | open | Continue route-family conversions for health/performance/maps-script/properties-marketing remaining raw errors or document intentional schemas per route. |
