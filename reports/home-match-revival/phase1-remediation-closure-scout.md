...
| DB P0.4: Fix interaction UNIQUE to `(user_id, property_id)` | Audit recommends migration, but current migration search found no constraint replacement for `user_property_interactions`; no closure commit found. | open | Add migration to drop `(user_id, property_id, interaction_type)` unique and add `(user_id, property_id)`; update tests. |
...
| Item | Evidence present | Status | Exact next action |
| --- | --- | --- | --- |
| M1: Fix middleware matcher exclusions (`_next/data`, static extensions) | `middleware.ts` matcher still excludes only `_next/static`, `_next/image`, favicon, and image extensions. | open | Expand matcher to exclude `_next/data`, HMR, CSS/JS, JSON/XML/TXT/fonts; add middleware matcher test. |
...
| M3: Add Cache-Control headers to GET API endpoints | Search shows only select routes have Cache-Control; audit said 24/27 missing; no broad closure. | open | Add route-specific cache policy headers and tests for user-specific no-store/no-cache. |
...
| M5: Add rate limiting to unprotected POST/admin routes | Audit said 19/27 missing; no broad closure evidence. | open | Inventory mutating/admin routes and add consistent limiter tests. |
