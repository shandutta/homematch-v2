...
| M6: Standardize API error handling | `ApiErrorHandler` now covers shared helpers plus interactions, Zillow random-image, couples/vibes, paid Maps proxy, and remaining couples activity/notify/disputed route families. | open | Continue route-family conversions for admin/health/performance/maps-script/properties-marketing remaining raw errors or document intentional schemas per route. |
