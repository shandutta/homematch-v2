...
| M3: Add Cache-Control headers to GET API endpoints | Advanced across authenticated/user-specific GET routes: `noStoreJson` now covers couples activity/check-mutual/disputed/mutual-likes/stats, interactions, neighborhoods vibes, properties vibes, and users search. Static Jest coverage verifies each route imports `@/lib/api/cache-control` and uses `noStoreJson`. | partial | Remaining classification is for unauthenticated/public/admin/metrics routes: maps scripts/proxies, random image, performance metrics, admin GET, and error responses where explicit no-store may still be appropriate. |
