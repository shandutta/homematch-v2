...
| M5: Add rate limiting to unprotected POST/admin routes | Partial closure: added route-scoped limiter coverage for authenticated couples notify, couples disputed PATCH, interactions DELETE, and IP-based performance metrics POST; `rate-limit-coverage.test.ts` verifies RED/GREEN route coverage. Admin cron-secret endpoints remain open. | partial | Next: add repo-only limiter coverage to admin cron routes without logging secrets; durable production limiter storage remains a separate decision. |
