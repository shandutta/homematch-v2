...
| M3: Add Cache-Control headers to GET API endpoints | Partially closed in this slice: added `src/lib/api/cache-control.ts` and applied `private, no-store, no-cache, must-revalidate, max-age=0` to user-specific couples GET routes `activity` and `mutual-likes`; test `cache-control.test.ts`. Audit still found many other GET routes without explicit policy. | partial | Continue route-by-route: classify user-specific routes as no-store, public/static proxy routes as bounded public cache, and health/metrics as no-store/no-cache. |
