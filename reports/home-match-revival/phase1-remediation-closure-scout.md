...
| M5: Add rate limiting to unprotected POST/admin routes | Scout artifact `rate-limit-gap-scout.md` created. Key gaps remain: admin status-refresh/ingest/generate-vibes/generate-neighborhood-vibes/generate-vibes-zillow; couples notify/disputed PATCH; interactions DELETE; performance metrics POST. | open | Remediate with existing limiter primitives route-by-route; durable production limiter storage remains a separate decision. |
