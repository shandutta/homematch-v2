...
| M11: Move/guard test-only exports in production code | Partial closure wave: removed the optional-user `__isMissingSupabaseConfigError` test-only export and shifted coverage to public `getOptionalServerUser` behavior plus a static no-`__` regression; RED `hm-m11-optional-red-1778212210.service`; GREEN `hm-m11-optional-green-1778212243.service`; type-check `hm-m11-optional-typecheck-1778212248.service`. Remaining `__withRefreshRecovery` exports still require a separate shared-helper refactor; SecureMapLoader reset helper remains test-env guarded. | partial | Continue with `__withRefreshRecovery` extraction or explicit gate decision; do not mark M11 fully closed yet. |
