...
- Closed: httpOnly Supabase cookies; Maps auth hardening; local dev guard bypass; Docker optional decision; COOP/CORP; properties RLS read-policy hardening; SECURITY DEFINER search-path hardening; property stats RPC; interaction uniqueness migration; middleware matcher exclusions; successful GET route cache-policy classification; M5 route-scoped limiter coverage for identified mutating/admin gaps; M6 shared error helper coverage; M6 interactions/zillow/couples-vibes/maps/remaining-couples/admin route conversion; service-role-client bypass gate; partial schema safety constraints.
- Open: auth client consolidation partial; removal of factory async stub; interactions service-role fallback; duplicate auth getUser monkey-patching; E2E auth lifecycle; password config alignment; cached service-client key rotation; query dedupe; realtime N+1; rollback/DOWN scripts; user_profiles DELETE policy; households INSERT policy; JSONB indexes; inline DB typing cleanup; API error standardization route conversion; timeout cleanup; dead code/dependency cleanup; README closure.
