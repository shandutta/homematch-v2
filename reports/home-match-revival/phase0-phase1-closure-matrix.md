...
- Open: E2E auth lifecycle; remaining dependency cleanup decision; pg_trgm text-search index if still required; README/local-dev validation cleanup; anonymous public-page fast path; route-deadline helper for long Supabase-heavy APIs; API auth smoke live execution with approved local/non-production bearer token.
