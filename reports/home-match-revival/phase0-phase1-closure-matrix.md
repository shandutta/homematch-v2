...
- Closed: httpOnly Supabase cookies; Maps auth hardening; local dev guard bypass; Docker optional decision; COOP/CORP; properties RLS read-policy hardening; SECURITY DEFINER search-path hardening; property stats RPC; service-role-client bypass gate; partial schema safety constraints.
- Open: auth client consolidation; removal of factory async stub; interactions service-role fallback; duplicate auth getUser monkey-patching; E2E auth lifecycle; password config alignment; cached service-client key rotation; interaction unique constraint; query dedupe; realtime N+1; rollback/DOWN scripts; user_profiles DELETE policy; households INSERT policy; JSONB indexes; inline DB typing cleanup; middleware matcher exclusions; cache headers; mutating-route rate limiting; API error standardization; timeout cleanup; dead code/dependency cleanup; README closure.
