...
- Final JSON route-family targeted: `hm-m6-final-json-green-1778210177.service` passed.
- Type-check: `hm-m6-typecheck-1778207696.service`, `hm-m6-interactions-typecheck3-1778208190.service`, `hm-m6-couples-typecheck-1778209637.service`, `hm-m6-admin-typecheck-1778209919.service`, and `hm-m6-final-json-typecheck-1778210186.service` passed.
- Diff check: passed.
...
- Converted final JSON route-family raw errors in `health`, `users/search`, `properties/marketing`, `performance/metrics`, `maps/script`, and `maps/metro-boundaries` to `ApiErrorHandler`.
- `maps/proxy-script` intentionally remains a JavaScript response endpoint with JavaScript comment bodies, not JSON API error envelopes.
- Regression coverage added to `__tests__/unit/api/error-standardization.test.ts`.

## Remaining M6 work

- JSON API route-family conversion is closed repo-side; broader Phase 0/1 blockers remain outside M6.
