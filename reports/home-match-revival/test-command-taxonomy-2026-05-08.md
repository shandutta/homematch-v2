...
`scripts/pre-commit-hook.js` is wired via `simple-git-hooks` and runs `pnpm run format && pnpm run lint:fix && pnpm exec eslint . --max-warnings=0 && pnpm run type-check` over the _whole_ repo on every commit, with a Codex auto-fix retry. That is fine in interactive use, but is not safe inside a bounded worker:
