-- Harden the create_household_for_user RPC to avoid foreign key failures
-- when the auth user record is missing (stale session) and to ensure the
-- user_profile row exists before linking a household.
...
COMMENT ON FUNCTION public.create_household_for_user() IS
  'Creates a household for the current user, validating auth user presence and provisioning user_profiles before linking.';
