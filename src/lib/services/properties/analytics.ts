...
export interface IPropertyAnalyticsService {
  ...
  getMarketTrends(timeframe: 'weekly' | 'monthly' | 'quarterly'): Promise<MarketTrends[]>
  analyzePropertyPricing(propertyId: string): Promise<PriceAnalysis | null>
  getPropertyTypeDistribution(neighborhoodId?: string): Promise<Record<string, number>>
  getPriceHistogram(neighborhoodId?: string, bins?: number): Promise<Array<{range: string; count: number}>>
  getTopPerformingProperties(metric: 'views' | 'likes' | 'price', limit?: number): Promise<Property[]>
  getMarketComparisons(propertyId: string, radius?: number): Promise<Property[]>
  getSimilarProperties(propertyId: string, radius?: number, limit?: number): Promise<Property[]>
}

export class PropertyAnalyticsService extends BaseService implements IPropertyAnalyticsService {
  ...
  async getPropertyStats(): Promise<PropertyStats> {
    return this.executeQuery(
      ...
      async (supabase) => {
        ...
        const avgPrice = prices.reduce((sum, price) => sum + price, 0) / totalProperties
        const medianPrice = prices[Math.floor(totalProperties / 2)]

        // Room statistics
        const avgBedrooms = properties.reduce((sum, p) => sum + p.bedrooms, 0) / totalProperties
        const avgBathrooms = properties.reduce((sum, p) => sum + p.bathrooms, 0) / totalProperties

        // Square feet statistics (excluding null values)
        const propertiesWithSquareFeet = properties.filter(p => p.square_feet !== null)
        const avgSquareFeet = propertiesWithSquareFeet.length > 0
          ? propertiesWithSquareFeet.reduce((sum, p) => sum + (p.square_feet || 0), 0) / propertiesWithSquareFeet.length
          : 0
    ...
    return this.executeQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          ...
          this.handleSupabaseError(error, 'getNeighborhoodStats', { neighborhoodId })
        }
        ...
        return {
          ...
          price_range: { min: result.min_price as number, max: result.max_price as number },
   ...
   */
  async getMarketTrends(timeframe: 'weekly' | 'monthly' | 'quarterly'): Promise<MarketTrends[]> {
    return this.executeQuery(
      'getMarketTrends',
      async (supabase) => {
        const rpc = createTypedRPC(supabase)
        const { data, error } = await rpc.get_market_trends({
          timeframe,
          months_back: timeframe === 'weekly' ? 3 : timeframe === 'monthly' ? 12 : 24
        })
   ...
   */
  async analyzePropertyPricing(propertyId: string): Promise<PriceAnalysis | null> {
    ...
    return this.executeQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          ...
          this.handleSupabaseError(error, 'analyzePropertyPricing', { propertyId })
        }
        ...
        if (comparableProperties.length > 0) {
          const avgComparablePrice = comparableProperties.reduce((sum, p) => sum + p.price, 0) / comparableProperties.length
   ...
   */
  async getPropertyTypeDistribution(neighborhoodId?: string): Promise<Record<string, number>> {
    return this.executeQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getPropertyTypeDistribution', { neighborhoodId })
        }
  ...
  async getPriceHistogram(
    ...
    return this.executeQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getPriceHistogram', { neighborhoodId, bins })
        }
  ...
  async getTopPerformingProperties(
    ...
    return this.executeArrayQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getTopPerformingProperties', { metric, limit })
        }
   ...
   */
  async getMarketComparisons(propertyId: string, radius = 5): Promise<Property[]> {
    this.validateRequired({ propertyId })
    ...
    return this.executeArrayQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getMarketComparisons', { propertyId, radius })
        }
  ...
  async getNeighborhoodPriceTrends(
    neighborhoodId: string,
    months = 12
  ): Promise<Array<{month: string; avg_price: number; listing_count: number}>> {
    this.validateRequired({ neighborhoodId })

    return this.executeQuery(
      'getNeighborhoodPriceTrends',
      async (supabase) => {
        ...
        if (error || !trends) {
          ...
          const monthlyTrends: Array<{month: string; avg_price: number; listing_count: number}> = []
          const now = new Date()
          ...
          for (let i = months - 1; i >= 0; i--) {
            ...
            const seasonalVariation = Math.sin((date.getMonth() / 12) * 2 * Math.PI) * 0.1
            const trendVariation = (months - i) * 0.02 // 2% yearly growth
            const randomVariation = (Math.random() - 0.5) * 0.1
            
            const avgPrice = Math.round(basePrice * (1 + seasonalVariation + trendVariation + randomVariation))
   ...
   */
  async getInvestmentAnalysis(propertyId: string): Promise<Record<string, unknown> | null> {
    this.validateRequired({ propertyId })
    ...
    return this.executeQuery(
      ...
      async (supabase) => {
        try {
          ...
          const futureValue5Year = purchasePrice * Math.pow(1 + appreciation5Year, 5)
          const totalReturn5Year = (futureValue5Year - purchasePrice + (annualRent * 5)) / purchasePrice * 100
          ...
          return {
            ...
            rental_analysis: {
              ...
              rent_to_price_ratio: Math.round((estimatedMonthlyRent / purchasePrice) * 10000) / 100
            },
            market_metrics: {
              avg_days_on_market: marketVelocity?.avg_days_on_market || 30,
              market_velocity_score: marketVelocity?.velocity_score || 50,
              liquidity_rating: (marketVelocity?.velocity_score ?? 50) > 70 ? 'high' : 
                               (marketVelocity?.velocity_score ?? 50) > 40 ? 'medium' : 'low'
            },
            investment_projections: {
              five_year_appreciation: Math.round((futureValue5Year - purchasePrice) * 100) / 100,
              five_year_total_return_percent: Math.round(totalReturn5Year * 100) / 100,
              break_even_years: Math.round((purchasePrice * 0.06) / annualRent * 10) / 10, // 6% carrying costs
              cash_flow_monthly: estimatedMonthlyRent - Math.round(purchasePrice * 0.005) // Est. monthly costs
            },
            risk_assessment: {
              price_volatility: priceAnalysis.market_position === 'above' ? 'high' : 
                              priceAnalysis.market_position === 'below' ? 'low' : 'medium',
              market_risk: (marketVelocity?.velocity_score ?? 50) < 30 ? 'high' : 
                          (marketVelocity?.velocity_score ?? 50) > 70 ? 'low' : 'medium',
              overall_risk_score: Math.round(((grossYield * 10) + (marketVelocity?.velocity_score || 50)) / 2)
            },
    ...
    return this.executeQuery(
      ...
      async (supabase) => {
        ...
        if (error || !data) {
          ...
          console.warn('Market velocity calculation failed, using fallback data:', error?.message)
          return {
   ...
   */
  async getSimilarProperties(propertyId: string, radius = 5, limit = 10): Promise<Property[]> {
    this.validateRequired({ propertyId })
    ...
    return this.executeArrayQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getSimilarProperties', { propertyId, radius, limit })
        }
