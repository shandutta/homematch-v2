...
import type { INeighborhoodService, ISupabaseClientFactory } from '@/lib/services/interfaces'
import { BaseService } from '@/lib/services/base'
import { createTypedRPC } from '@/lib/services/supabase-rpc-types'

export class NeighborhoodService extends BaseService implements INeighborhoodService {
   ...
   */
  async createNeighborhood(neighborhood: NeighborhoodInsert): Promise<Neighborhood | null> {
  ...
  async updateNeighborhood(
    ...
    return this.executeSingleQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'updateNeighborhood', { neighborhoodId, updates })
        }
   ...
   */
  async getNeighborhoodsByCity(city: string, state: string): Promise<Neighborhood[]> {
    ...
    return this.executeArrayQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getNeighborhoodsByCity', { city, state })
        }
   ...
   */
  async getNeighborhoodsByMetroArea(metroArea: string): Promise<Neighborhood[]> {
    ...
    return this.executeArrayQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getNeighborhoodsByMetroArea', { metroArea })
        }
   ...
   */
  async getNeighborhoodsWithStats(city?: string, state?: string): Promise<Record<string, unknown>[]> {
    return this.executeArrayQuery(
      ...
      async (supabase) => {
        ...
        if (error) {
          this.handleSupabaseError(error, 'getNeighborhoodsWithStats', { city, state })
        }
   ...
   */
  async getNeighborhoodAnalytics(neighborhoodId: string): Promise<Record<string, unknown> | null> {
    this.validateRequired({ neighborhoodId })
    ...
    return this.executeQuery(
      ...
      async (supabase) => {
        ...
        if (neighborhood.error) {
          this.handleSupabaseError(neighborhood.error, 'getNeighborhoodAnalytics', { neighborhoodId })
        }
        ...
        const avgBedrooms = propertyData.reduce((sum, p) => sum + p.bedrooms, 0) / totalProperties
        const avgBathrooms = propertyData.reduce((sum, p) => sum + p.bathrooms, 0) / totalProperties

        const propertiesWithSquareFeet = propertyData.filter(p => p.square_feet !== null)
        ...
        const typeDistribution = propertyData.reduce((acc, p) => {
  ...
  async deleteNeighborhood(neighborhoodId: string): Promise<boolean> {
    ...
    return this.executeBooleanQuery(
      ...
      async (supabase) => {
        ...
        if (countError) {
          this.handleSupabaseError(countError, 'deleteNeighborhood', { neighborhoodId })
        }

        if (count && count > 0) {
          throw new Error(`Cannot delete neighborhood: ${count} properties still reference it`)
        }
        ...
        if (error) {
          this.handleSupabaseError(error, 'deleteNeighborhood', { neighborhoodId })
        }
