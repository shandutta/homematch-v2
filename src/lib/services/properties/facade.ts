...
import { PropertyAnalyticsService, type IPropertyAnalyticsService } from './analytics'
 ...
 */
export class PropertyServiceFacade extends BaseService implements IPropertyService {
  ...
  async getPropertyWithNeighborhood(propertyId: string): Promise<PropertyWithNeighborhood | null> {
  ...
  async updateProperty(propertyId: string, updates: PropertyUpdate): Promise<Property | null> {
  ...
  async getPropertiesWithinRadius(
    ...
    return this.geographicService.getPropertiesWithinRadius(latitude, longitude, radiusKm)
  }
  ...
  async updateNeighborhood(neighborhoodId: string, updates: NeighborhoodUpdate): Promise<Neighborhood | null> {
  ...
  async getPropertiesAlongRoute(waypoints: Array<{lat: number; lng: number}>, corridorWidth: number): Promise<Property[]> {
    return this.geographicService.getPropertiesAlongRoute(waypoints, corridorWidth)
  }

  async getGeographicDensity(bounds: {
    northEast: { lat: number; lng: number }
    southWest: { lat: number; lng: number }
  }, gridSize: number): Promise<{
  ...
  async getCommuteAnalysis(propertyId: string, destinations: Array<{lat: number; lng: number}>): Promise<Record<string, unknown> | null> {
 ...
 */
export function createPropertyService(clientFactory?: ISupabaseClientFactory): PropertyServiceFacade {
