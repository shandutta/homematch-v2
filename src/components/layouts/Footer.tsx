            ...
            <div className="flex flex-wrap gap-4">
              {legalLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-hm-stone-500 transition hover:text-white"
                >
                  {link.label}
                </Link>
              ))}
      ...
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
        <div className="border-hm-stone-600/60 bg-hm-obsidian-900/80 relative overflow-hidden rounded-[32px] border px-6 py-12 shadow-[0_20px_55px_rgba(52,43,37,0.10)] backdrop-blur-2xl sm:px-10">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            ...
            <Link
              href="/profile"
              className="group border-hm-stone-600/60 bg-hm-obsidian-800 text-hm-stone-200 inline-flex items-center gap-2 rounded-full border px-5 py-3 text-sm font-semibold transition hover:-translate-y-0.5 hover:border-cyan-200/70 hover:bg-white/20"
            >
              Update profile
            ...
            {visibleHighlightLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className="border-hm-stone-600/60 bg-hm-obsidian-900/70 hover:bg-hm-obsidian-800/70 flex flex-col rounded-2xl border p-5 transition duration-200 hover:border-cyan-200/40"
                >
                  <div className="bg-hm-obsidian-800 rounded-2xl p-3 text-cyan-200">
